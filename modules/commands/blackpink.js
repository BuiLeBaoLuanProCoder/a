...
var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/cache/bannerwe.jpeg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/bannerwe.jpeg"),event.messageID);
	 return request(encodeURI(`https://api-alphabot.herokuapp.com/api/textpro/blackpink?text=${namee}&apikey=Alphabot`)).pipe(fs.createWriteStream(__dirname+'/cache/bannerwe.jpeg')).on('close',() => callback());     
}