...
const time = process.uptime(),
  ...
  let k = args[0];
   if(args[0] == "list"){
    const alime = (await axios.get('https://run.mocky.io/v3/23219a84-1561-4e87-ae1b-7c15bdb3c13a')).data
  ...
  var id = Math.floor(Math.random() * 848) +1
    ...
    const lengthchar = (await axios.get('https://run.mocky.io/v3/a5e1f962-a309-4874-98f0-1c0e3ea169c7')).data
...
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "45px time";
    ctx.fillText("@" + "hanaku.user", 930, 540)
    ctx.fillText("@" + "hanakuUwU", 930, 610)
    ctx.fillText("@" + "hanaku.user", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
