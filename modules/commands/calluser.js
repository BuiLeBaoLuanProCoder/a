...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  ...
  var formReport = `[ Hi Mình Là Bot Đưa Thư Và Có Một Lá Thư Gửi Đến Cho Bạn Đây ]\n\n» Nội dung: ${reason}\n\nTime: ${gio}` // gui tin nhan
  ...
  if (event.type == "message_reply") {
    ...
    api.sendMessage(`» Vào ${gio}\n» Lời nhắn của bạn đã được gửi đến người nào đó `, threadID, () => {
      api.sendMessage(formReportPath,`${id}`, (error, info) => {
        if (error) { return api.sendMessage('Gửi Không Thành Công', event.threadID, event.messageID) } else
