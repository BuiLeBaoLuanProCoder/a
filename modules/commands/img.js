...
const name = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[1] || "";
    ...
    } else if(owo == "list" || owo == "ds"){
      const opp = JSON.parse(fs.readFileSync(__dirname + "/tad/data2.json"))
      var count = opp.listAnime.length;
      var data = opp.listAnime;
            var page = 1;
      page = parseInt(name) || 1;
      page < -1 ? page = 1 : "";
      var limit = 20;
  ...
  const lines = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[3] || "";
   ...
   const v_color = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[5] || "#63F2D8"
   ...
   const lengthchar = (await axios.get('https://run.mocky.io/v3/c3d88a1a-56e6-4d7b-88a9-e936a4eb1e0c')).data
    ...
    ctx.fillStyle = "#ffff";
    ctx.font = "35px time"
    const xyz = await this.wrapText(ctx, title, 300);
