module.exports.config = {
    ...
    version: "1.1.1",
    hasPermssion: 0,
    credits: "Mirai Team mod cực mạnh by araxy",
...
var content = args.slice(1);
 if(!fs.existsSync(__dirname+'/cache/SplineSans-Medium.ttf')) { 
    ...
    let bg = (await axios.get(`https://i.imgur.com/qvMnrq2.jpg`, {responseType: "arraybuffer" })).data;
...
if(!isNaN(args[0])) {
    ...
    ctx.textAlign = "start";
    ctx.fillText('' + args[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.') + 'đ', 145, 470);
    ctx.font = "25px SplineSans";
    ctx.fillText(`${content}`, 64, 900)
    ctx.font = "25px SplineSans-Medium";
    ctx.textAlign = "right";
    ctx.fillText(`${name}`, 265, 50);
