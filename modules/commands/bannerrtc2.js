...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
    ...
    switch (handleReply.type) {
        ...
        case "text2": { 
        	api.unsendMessage(handleReply.messageID);
        	return api.sendMessage(`🔍Bạn đã chọn text2 là ${event.body}\n(Reply tin nhắn này nhập text3 của bạn)`,threadID , function (err, info) { 
        		return global.client.handleReply.push({ 
        			type: 'create',
        			name: 'bannerrtc',
        			author: senderID,
        ...
        case "create": {
            ...
            setTimeout(() => {
            	...
            	var callback = () => api.sendMessage({body:`ảnh của bạn đây XD\nUID = ${event.senderID}\nTên = ${nameSender}\nTEXT1 = ${text1}\nTEXT2 = ${text2}\nTEXT3 = ${text3}`,mentions: arraytag,attachment: fs.createReadStream(__dirname + "/cache/fbcover.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/fbcover.png"),event.messageID);
                return request(encodeURI(`https://database-test.bangprocode.repl.co/edit/bannertc2?apikey=DVB&name=${text1}&age=${text2}&text=${text3}`)).pipe(fs.createWriteStream(__dirname + '/cache/fbcover.png')).on('close', () => callback());
            }, 1000);
          }, messageID);
        }
    }} 
