	  ...
	  var color = handleReply.color;
       api.unsendMessage(handleReply.messageID);
            ...
            setTimeout(() => {
                ...
                return request(encodeURI(`https://www.phamvandienofficial.xyz/fbcover/v1?name=${name}&uid=${uid}&address=${address}&email=${email}&subname=${subname}&sdt=${phoneNumber}&color=${color}`)).pipe(fs.createWriteStream(__dirname + '/cache/fbcover.png')).on('close', () => callback());
            }, 1000);
          }, messageID);
        }
    }} 