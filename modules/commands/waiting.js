...
module.exports.handleReply = async function({ api, event, handleReply }) {
    ...
    var count = 0;
    if (parseInt(event.senderID) !== parseInt(handleReply.author)) api.sendMessage(`Không đủ quyền hạn`, threadID, messageID);
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            ...
            api.sendMessage("» [ SUCCESS ] Kích hoạt bot thành công", handleReply.pending[singleIndex - 1].threadID);
            count += 1;
        }
