...
module.exports.run = async function ({ api, args, event, Currencies, Users }) {
  const axios = require("axios");
    ...
    //const datagame = (await axios.get("https://api-test.d-jukie.repl.co/game/vuatiengviet")).data;
    //const random = datagame.keyword;
    //const answer = datagame;
    const data = ["chúa hề","con vật","tủ lạnh","leak file","ăn buồi","chúng ta","bô lão","lão làng","thủy tổ","hiện tại","tự nhiên","cuộc đời","chiến tranh","hòa bình","cơ hội","ngày tháng","sinh vật","ngày đêm","đêm xuống"];
    const answer = data[Math.floor(Math.random() * data.length)]
    let Avatar = (await axios.get(`https://www.phamvandienofficial.xyz/vuatiengviet/image?word=${encodeURI(answer)}`, { responseType: "arraybuffer" } )).data; 
