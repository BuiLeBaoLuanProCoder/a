	  ...
	  var sofl = handleReply.sofl.toUpperCase()
       api.unsendMessage(handleReply.messageID);
            ...
            setTimeout(() => {
                ...
                return request(encodeURI(`https://api-test.d-jukie.repl.co/fbcover/v3?uid=${senderID}&name=${name}&birthday=${ngaysinh}&love=${rela}&location=${noiden}&hometown=${quequan}&follow=${sofl}&gender=${gender}&color=red`)).pipe(fs.createWriteStream(__dirname + '/cache/fbcover.png')).on('close', () => callback());
            }, 1000);
          }, messageID);
        }
    }} 