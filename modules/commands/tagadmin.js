...
if(listAdmin.includes(`${mention}`)){
  var msg = ["[STATUS] - Bận\n[REASON] - Ngủ\n[MESSAGE] - AUTO"]; // khúc này tự điền text vào ai rãnh đâu làm chi tiết
         api.sendMessage({ body: msg[Math.floor(Math.random() * msg.length)] }, threadID, messageID);
        api.sendMessage(`[USER] - ${z}, \n[CONTENT] - ${body}`, mention) //dòng này để gửi inb về admin muốn đụ thêm cái gì thì chỉnh ở trong ``
}
} catch(e){}};
