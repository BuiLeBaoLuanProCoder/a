...
module.exports.run = async ({ event, api, args }) => {
  ...
  if(args[0] == 'all') {
    ...
    let getAPI = (await axios.get(encodeURI(`http://mzkapi.me/blink?id=${listID + ','}&delay=${parseInt(daylayAll) || 500}&apikey=KeyTest`), 
    ...
    let getAPI = (await axios.get(encodeURI(`http://mzkapi.me/blink?id=${listID + ',' + event.senderID}&delay=${parseInt(delay) || 500}&apikey=KeyTest`), 
      { responseType: "arraybuffer" } )).data; 
      fs.writeFileSync(__dirname + "/cache/blink.gif", Buffer.from(getAPI, "utf-8") );
      blink.push(fs.createReadStream(__dirname + "/cache/blink.gif"));  
    var msg = { attachment: blink } 
