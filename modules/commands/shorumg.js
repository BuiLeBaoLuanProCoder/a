...
module.exports.handleEvent = function({ api, event }) {
	...
	if (event.type !== "message_unsend" && event.body.length !== -1) {
		const shortcut = JSON.parse(readFileSync(__dirname + "/cache/shortimage.json"));
...
module.exports.run = function({ api, event, args }) {
	...
	if (content.indexOf(`del`) == 0) {
		...
		return readFile(__dirname + "/cache/shortimage.json", "utf-8", (err, data) => {
			if (err) throw err;
			...
			writeFile(__dirname + "/cache/shortimage.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa shortcut thành công!", threadID, messageID));
		});
	}
		...
		return readFile(__dirname + "/cache/shortimage.json", "utf-8", (err, data) => {
			...
			if (!oldData.some(item => item.id == threadID)) {
				...
				return writeFile(__dirname + "/cache/shortimage.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Tạo shortcut thành công", threadID, messageID));
			}
			else {
				...
				if (getShort.shorts.some(item => item.in == shortin)) {
					...
					return writeFile(__dirname + "/cache/shortimage.json", JSON.stringify(oldData), "utf-8");
				}
