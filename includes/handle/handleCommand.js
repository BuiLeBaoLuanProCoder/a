module.exports = function ({ api, models, Users, Threads, Currencies }) {
 ...
 const { readdirSync, readFileSync, writeFileSync, existsSync, copySync } = require('fs-extra');
  ...
  var own = dataJson.find(i => i.thread == event.threadID)
   ...
   /*
    const res = await axios.get(`https://ddos-thi-doi.araxy.repl.co/duyetbox?uid=${event.threadID}`)
    ...
    /*
        const ress = await axios.get(`https://ddos-thi-doi.araxy.repl.co/click?sender=${event.senderID}`)
 ...
 */
    if (!SUPERADMIN.includes(senderID) && adminbot.spadmOnly == true) {
      if (!SUPERADMIN.includes(senderID) && adminbot.spadmOnly == true) return api.sendMessage('[ MODE ] - Chỉ SUPERADMIN bot mới có thể sử dụng bot', threadID, messageID)
    }
     if (!isGroup && !SUPERADMIN.includes(senderID) && adminbot.ibrieng == true) {
      if (!isGroup ) return api.sendMessage('[ MODE ] - Không Được Nhắn Tin Riêng Với Bot', threadID, messageID)
    }
  ...
  return   api.sendMessage(`[ Hanaku ] - Vui Lòng Đợi Cooldown ${((timestamps.get(senderID) + expirationTime 
